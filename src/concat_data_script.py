...
if __name__ == "__main__":
    # Concatenate the data for each logger. For example,
    # concat_logger_data(
    #     logger_name="0ff4",
    #     raw_data_dir="raw_data/data_2025-02-09/data/",
    #     output_data_dir="data/winter-term-packard/"
    # )
    ...
    output_data_dir = pathlib.Path(
        "data/pepperwood_2025_gdrive/pepperwood_2025_gdrive_10-23_19-54")

    if len(sys.argv) > 1:
        logger_name = sys.argv[1]
        # The path may be a windows path, so we need to treat it as a raw
        # string to avoid escape characters
        raw_data_dir = pathlib.Path(r"{}".format(sys.argv[2]))
        output_data_dir = pathlib.Path(r"{}".format(sys.argv[3]))
